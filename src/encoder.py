...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        att_output = self.attention(hidden_state, hidden_state, hidden_state, mask)
        x = x + att_output
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, vocab_size: int, max_position_embeddings: int, d_model: int,
        ...
        self.layers = nn.ModuleList([TransformerEncoderLayer(d_model, num_attention_heads, intermediate_size) for _ in range(num_attention_heads)])
