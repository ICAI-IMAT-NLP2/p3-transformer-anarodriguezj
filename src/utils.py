...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        d_q = d_v = d_k= d_model // num_attention_heads

        self.heads = nn.ModuleList([AttentionHead(d_model, d_k, d_q, d_v) for _ in range(num_attention_heads)])
    ...
    def forward(self, x_q, x_k, x_v, mask=None):
        ...
        x = [head(x_q, x_k, x_v, mask) for head in self.heads]
