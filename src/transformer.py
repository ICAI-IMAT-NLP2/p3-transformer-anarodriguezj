...
class Transformer(nn.Module):
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                for next_token in range(log_probs.size(1)):
                    new_seq = torch.cat([seq, torch.tensor([[next_token]], device=device)], dim=1)  # Shape: (1, seq_len+1)
    ...
    def __sampling_decode(self, src_input: torch.Tensor, max_length: int, temperature: float = 1.0, **kwargs) -> torch.Tensor:
        ...
        # Initialize the target sequence with SOS_token
        tgt_input = torch.full((batch_size, 1), SOS_token, dtype=torch.long, device=device)  # Shape: (batch_size, 1)
        ...
        for _ in range(max_length):
            ...
            tgt_input = torch.cat((tgt_input, next_token), dim=1)  # Shape: (batch_size, seq_len + 1)
    ...
    def __top_k_sampling_decode(self, src_input: torch.Tensor, max_length: int, k: int = 10, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            topk_log_probs, topk_indices = torch.topk(log_probs, k, dim=-1)  # Shape: (batch_size, k)
            # Sample from the top k tokens
            probs = torch.exp(topk_log_probs) # Shape: (batch_size, 1)
            probs = probs / probs.sum(dim=-1, keepdim=True)  # Normalize to get probabilities
            next_token = torch.multinomial(probs, num_samples=1)  # Shape: (batch_size, 1)
            # Map sampled indices to original token indices
            next_token = topk_indices.gather(-1, next_token)  # Shape: (batch_size, 1)
    ...
    def __top_p_sampling_decode(self, src_input: torch.Tensor, max_length: int, p: float = 0.9, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            sorted_probs, sorted_indices = torch.sort(probs, descending=True, dim=-1)  # Shape: (batch_size, vocab_size)
            ...
            next_token = torch.multinomial(sorted_probs, num_samples=1)  # Shape: (batch_size, 1)
            # Map sampled indices to original token indices
            next_token = sorted_indices.gather(-1, next_token)  # Shape: (batch_size, 1)
            # Append next token to tgt_input
            tgt_input = torch.cat((tgt_input, next_token), dim=1)  # Shape: (batch_size, seq_len + 1)
            # Check if all sequences have generated EOS_token
            if (next_token == EOS_token).all():
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        # Initialize the target sequence with SOS_token
        tgt_input = torch.full((1, 1), SOS_token, dtype=torch.long, device=device)  # Shape: (1, 1)
        ...
        for _ in range(max_length):
            ...
            probs = F.softmax(logits, dim=-1)  # Shape: (batch_size, vocab_size)
            # Get the top k tokens
            topk_probs, topk_indices = torch.topk(probs, k, dim=-1)  # Shape: (batch_size, k)

            # Prepare tensors for all candidates
            expanded_tgt_input = tgt_input.repeat(k, 1)  # Shape: (k, seq_len)
            next_tokens = topk_indices[0].unsqueeze(-1)  # Shape: (k, 1)
            y_candidates = torch.cat([expanded_tgt_input, next_tokens], dim=1) # Shape: (k, seq_len + 1)

            # Pass each candidate through the decoder
            dec_outputs_candidate = self.decoder(y_candidates, enc_output.repeat(k, 1, 1))  # Shape: (k, seq_len + 1, d_model)
            ...
            h_v_norm = F.normalize(h_v)  # Shape: (k, hidden_size)
            h_j_norm = F.normalize(h_j)  # Shape: (k, seq_len, hidden_size)

            # Compute cosine similarities between h_v and each h_j
            cos_sim = F.cosine_similarity(h_v_norm.unsqueeze(1), h_j_norm)  # Shape: (k, seq_len)

            # Get maximum cosine similarity for each candidate
            max_sim = cos_sim.max(dim=-1)[0]  # Shape: (k,)

            # Compute scores
            P_LM_v = torch.log(topk_probs.view(-1))  # Shape: (k,)
            scores = alpha * P_LM_v - (1 - alpha) * max_sim  # Shape: (k,)

            # Select the candidate with the highest score
            best_idx = scores.argmax()
            best_token =  y_candidates[best_idx, -1].unsqueeze(0).unsqueeze(0)   # Shape: (1, 1)
        ...
        generated_sequence = tgt_input[:, 1:]  # Shape: (1, seq_len)
        return generated_sequence
